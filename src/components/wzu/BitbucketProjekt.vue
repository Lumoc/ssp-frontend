        ...
        <form v-on:submit.prevent="addToBackendBitbucket">
            ...
            <b-field label="Project Key (only uper case letters starting with the domain model)"
                ...
                <b-input v-model.trim="bitprojectkey"
                         name="Projekt Key"
                         v-validate.rules="{ required: true, regex: /^((AMG|AML|AMN|AMP|ARD|AM|BP|EK|EN|ERP|FCON|FI|FT|HR|LOG|IT|INV|KD|KI|KIP|KIN|KS|MV|MVC|MVG|MVP|MVL|NM|NMD|OM|PN|PPG|PPP|PT|PV|PZ|RSW|SC|TA|TC|TP|TEST|TMS|USER|WP|ZF)_?.*)$/}">
                </b-input>
            </b-field>
